...
export class LovComponent implements OnInit {
  ...
  ngAfterViewInit(): void {
    this.translateService.stream('ApplicationTitle').subscribe(value => {
      this.translateService.stream('Lov.PageTitle').subscribe(valueTitle => {
  ...
  Delete(obj: any) {
    ...
    _this.app.confirmationService.confirm({
      message: _this.translateService.instant('Confirmation.deleteMsg') + ": " + obj.lovCode,
      ...
      accept: () => {
        ...
        _this.lovService.DeleteLov(obj.id).subscribe(function (data) {
          var response = data as Response<any>;
          if (response.succeeded) {
            let index = _this.lovs.findIndex(d => d.lovId === obj.lovId);
            _this.lovs.splice(index, 1);
  ...
  getAllLov() {
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback, settings) => {
        ...
        this.lovService
          .GetAllLovFunctional(dataTablesParameters)
          .subscribe((resp) => {
            if (resp['succeeded']) {
              this.dtTrigger.next();
              this.lovs = resp['data'];
  ...
  Details(obj: any) {
    this.lovId = obj.id;
    this.lovCode = obj.lovCode;
    this.lovNameAr = obj.lovNameAr;
    this.lovNameEn = obj.lovNameEn;
    this.sqlStatement = obj.sqlStatement;
    this.update = 1;
  }
  save() {
    ...
    if (this.update == 0) {
      if (this.LovForm.valid) {
        this.preview = JSON.stringify(this.LovForm.value);
        var _this = this;
        _this.lovService.AddLov(this.preview).subscribe(function (data) {
          var response = data as Response<any>;
          if (response.succeeded) {
            _this.lovs.push(response.data)
            _this.reset();
      ...
      var obj = {
        ...
        "lovNameAr": this.lovNameAr,
        "lovNameEn": this.lovNameEn,
        "sqlStatement": this.sqlStatement,

      }
      var _this = this;
      _this.lovService.UpdateLov(obj).subscribe(function (data) {
        var response = data as Response<any>;
        if (response.succeeded) {
          _this.lovs.map((obj) => {
            if (obj.lovId === response.data.lovId) {
              obj.lovCode = response.data.lovCode;
              obj.lovNameAr = response.data.lovNameAr;
              obj.lovNameEn = response.data.lovNameEn;
              obj.sqlStatement = response.data.sqlStatement;
            }
          });
          _this.reset();
          _this.app.messageService.add({ severity: 'success', key: 'PlanValidation', summary: _this.translateService.instant('General.SavedSuccessfully'), detail: _this.translateService.instant('General.SavedSuccessfully'), life: 6000 });
