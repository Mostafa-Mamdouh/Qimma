...
import { DTOSafetyResponsible } from '../../../models/SafetyResponsible/SafetyResponsible';
import { SafetyResponsibleServices } from '../../../services/SafetyResponsible/SafetyResponsible.service';
...
@Component({
  selector: 'SafetyResponsibleOfficer',
  templateUrl: './SafetyResponsibleOfficer.component.html',
  styleUrls: ['./SafetyResponsibleOfficer.component.css']
})

export class SafetyResponsibleOfficerComponent implements OnInit, AfterViewInit {

  currentUser: UserData = <UserData>{};
  DataObject: DTOSafetyResponsible = <DTOSafetyResponsible>{};
  DataList: any;
  CountryList: any;
  CityList: any;
  ...
  SubmitForm = new FormGroup({
    SRONameAr : new FormControl (''),
    SRONameEn : new FormControl(''),
  ...
  constructor(
    ...
    private SafetyResponsibleServices: SafetyResponsibleServices,
  ...
  getData() {
    var _this = this;
    _this.SafetyResponsibleServices.GetAll().subscribe(function (data) {
      var response = data as Response<DTOSafetyResponsible[]>;
  ...
  Delete(obj: any) {
    ...
    _this.app.confirmationService.confirm({
      ...
      accept: () => {
        ...
        _this.SafetyResponsibleServices.Delete(obj.id).subscribe(function (data) {
  ...
  Details(obj: any) {
    var _this = this;
    _this.DataObject = <DTOSafetyResponsible>{};
    _this.DataObject.id = obj.id;
    _this.DataObject.AmanID = obj.AmanID;
    _this.DataObject.AmanInsertedOn = obj.amanInsertedOn;
    _this.DataObject.CertificateNo = obj.certificateNo;
    _this.DataObject.SRONameAr = obj.sroNameAr
    _this.DataObject.SRONameEn = obj.sroNameEn
    _this.DataObject.EmailId = obj.emailId
    _this.DataObject.IssuanceDate = obj.issuanceDate
    _this.DataObject.NationalID = obj.nationalID
    _this.DataObject.MobileNo = obj.mobileNo
    _this.DataObject.PhoneNo = obj.phoneNo
    _this.DataObject.ExpiryDate = obj.expiryDate
  ...
  save() {
    ...
    if (_this.Update == 0) {
      if (_this.SubmitForm.valid) {
        _this.SafetyResponsibleServices.Add(JSON.stringify(this.SubmitForm.value)).subscribe(function (data) {
          var response = data as Response<boolean>;
          if (response.succeeded && response.data) {
            _this.getData();
      ...
      _this.SafetyResponsibleServices.Update(JSON.stringify(this.SubmitForm.value)).subscribe(function (data) {
        var response = data as Response<boolean>;
        if (response.succeeded && response.data) {
          _this.getData();
          _this.SubmitForm.reset();
  ...
  ExportExel(): void {
    ...
    /* save to file */
    XLSX.writeFile(wb, "safetySheet.xlsx");
