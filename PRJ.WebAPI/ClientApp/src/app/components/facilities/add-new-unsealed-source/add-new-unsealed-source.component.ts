...
@Component({
  selector: 'app-add-new-unsealed-source',
  templateUrl: './add-new-unsealed-source.component.html',
  styleUrls: ['./add-new-unsealed-source.component.css'],
})
export class AddNewUnsealedSourceComponent implements OnInit {
  updateId: string;
  ...
  @ViewChild('numOfRadionuclides') numOfRadionuclides: any;
  ...
  addNewSourceForm = this.fb.group({
    ...
    facilitySourceID: ['', [Validators.maxLength(30)]],
    associatedEquipment: ['', Validators.required],
    ...
    if (transactionType.id == 2) {
      ...
      if (permit) {
        this.transactionService
          .GetSourcesByPermit(permit['id'], SourceTypes.unsealed)
          .subscribe((res) => {
            console.log(res);
  ...
  mapInputWithModel(body, sourceStatus): AddNewSourceModel | null {
    try {
      ...
      let sourceType = this.lookups['SourceType'].find((x) => x.value == 2).id;
