...
@Component({
  selector: 'app-variable-unsealed-transaction-edit',
  templateUrl: './variable-unsealed-transaction-edit.component.html',
  styleUrls: ['./variable-unsealed-transaction-edit.component.css'],
})
export class VariableUnsealedTransactionEditComponent implements OnInit {
  // On update attribute
  ...
  addNewSourceForm = this.fb.group({
    ...
    sourceActivity: [{ value: null, disabled: true }, Validators.required],
    sourceActivityUnit: [{ value: null, disabled: true }, Validators.required],
  ...
  setUpdateData(data) {
    ...
    if (data.noSourceTagImageFlag) {
      this.handleCertificateChange(1);
    } else if (data.noCharacterizationCertificateFlag) {
      this.handleCertificateChange(2);
    } else if (data.noManufacturerCertificateFlag) {
      this.handleCertificateChange(6);
    }
  }
  ...
  updateAttribute(oldValue: any, field: any) {
    ...
    } else if (field == TransactionAttributes.quantity) {
      this.oldValue = this.source.quantity;
    }
  }
  ...
  submit() {
    if (this.oldValue && this.newValue && this.purpose) {
      ...
      } else if (this.onUpdateAttribute == TransactionAttributes.quantity) {
        transactionTypeId = TransactionTypes.ChangeSourceQuantity;
        transactionAttribute = TransactionAttributes.quantity;
      }
